    ...
    try {
      ...
      if (loggedIn) {
        ...
        if (result && result.id) {
          ...
          config.remote = { playlistId: result.id, playlistName: name }
          repo.saveConfig(targetDir, config)
          repo.saveHead(targetDir, result.id)
          console.log(`已创建远程歌单: ${result.id}`)
        } else if (result && result.code !== 200) {
          console.log(`创建远程歌单失败: ${result.message || '未知错误'}`)
        }
      } else {
        console.log('未登录，跳过创建远程歌单。请先运行 ncmgit login')
      }
    } catch (err) {
